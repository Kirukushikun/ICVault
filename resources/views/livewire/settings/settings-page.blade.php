<div x-data="settingsPage()">
    <div class="mb-7">
        ...
        <div class="text-[12.5px] text-text-muted mt-1.5 font-light max-w-[560px]">
            Manage your account, quiz preferences, and notification rules.
        </div>
    </div>
    ...
    <div class="mb-10">
        ...
        <div class="flex items-center gap-4 pb-5.5 mb-1 border-b border-white/5">
            <div class="w-12 h-12 rounded-full shrink-0 flex items-center justify-center font-display text-lg font-bold shadow-[0_0_0_3px_rgba(195,7,63,0.2)]" style="background:linear-gradient(135deg,var(--color-red-dim),var(--color-red))">IC</div>
            <div>
                <div class="text-[15px] font-semibold">Iverson Craig</div>
                <div class="text-[11px] text-text-muted mt-0.5">Personal · Full Access</div>
            </div>
            <button @click="toggleEdit()" class="ml-auto text-[11px] font-semibold tracking-[0.08em] uppercase pb-1.5 border-b transition-colors"
                    :class="isEditing ? 'text-[#ec5c86] border-red/40' : 'text-text-muted border-transparent hover:text-white'"
                    x-text="isEditing ? 'Editing…' : 'Edit'"></button>
        </div>
        ...
        <div class="flex flex-col">
            <div class="flex items-center gap-4 py-3.5 border-b border-white/5">
                ...
                <input type="text" x-model="name" :disabled="!isEditing" class="flex-1 bg-transparent border-b py-1 text-[13px] focus:outline-none" :class="isEditing ? 'border-white/12 focus:border-red' : 'border-transparent opacity-50'" />
            </div>
            <div class="flex items-center gap-4 py-3.5 border-b border-white/5">
                <div class="w-[90px] shrink-0 text-[10px] font-bold tracking-[0.16em] uppercase text-text-muted">Email</div>
                <input type="email" x-model="email" :disabled="!isEditing" class="flex-1 bg-transparent border-b py-1 text-[13px] focus:outline-none" :class="isEditing ? 'border-white/12 focus:border-red' : 'border-transparent opacity-50'" />
            </div>
            <div class="flex items-center gap-4 py-3.5">
                ...
                <div class="flex-1 relative">
                    <input :type="passVisible ? 'text' : 'password'" x-model="password" :disabled="!isEditing" class="w-full bg-transparent border-b py-1 text-[13px] tracking-[0.2em] focus:outline-none" :class="isEditing ? 'border-white/12 focus:border-red' : 'border-transparent opacity-50'" />
                    <button @click="passVisible = !passVisible" class="absolute right-0 top-1/2 -translate-y-1/2 text-text-muted hover:text-white text-[13px]">👁</button>
                </div>
            </div>
        </div>

        <div x-show="isEditing" class="flex items-center gap-3.5 pt-4">
            <button @click="saveCredentials()" class="text-[11.5px] font-bold tracking-[0.08em] uppercase text-red hover:opacity-75">Save Changes</button>
            <button @click="cancelEdit()" class="text-[11.5px] text-text-muted hover:text-white tracking-[0.06em] uppercase">Cancel</button>
        </div>
    </div>

    {{-- Quiz Preferences --}}
    <div class="mb-10">
        <div class="flex items-center gap-3.5 text-[9.5px] font-bold tracking-[0.22em] uppercase text-white/28 mb-5.5 after:content-[''] after:flex-1 after:h-px after:bg-border">Quiz Preferences</div>
...
<script>
    Alpine.data('settingsPage', () => ({
        ...
        cancelEdit() {
            this.name = 'Iverson Craig';
            this.email = 'admin_it@bfcgroup.org';
            this.password = '••••••••••';
