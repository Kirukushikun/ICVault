...
was built, but nothing in it is specific to that project — reuse it anywhere. Currently applied
to **ICVault** (`c:\Users\DEV-PC\Desktop\Dump\ic-vault\`) as its first reuse outside PANDA.

> This is the *technical* method. It's paired with a **Development Plan Template** (this method,
> filled in per-project) and, where a supervisor/stakeholder needs an outcome-level status view,
> an **Asana Plan Template** — see `Development Plan Template.md` alongside this file. ICVault is
> solo/personal, so it skips the Asana companion for now; keep the filled-in Development Plan as
> the single source of truth (plus a `ui-scaffold-checklist.md` once Stage 2 starts).
